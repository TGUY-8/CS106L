...
void parse_csv(std::string filename, std::vector<Course>& courses) {
  std::ifstream file_parser(filename);  std::string line , header; std::vector<std::string> elements;
  std::getline(file_parser , header);
  while (std::getline(file_parser , line))
  {
    Course c;
    elements = split(line , ','); 
    line.clear();
    c.title = elements[0];
    c.number_of_units = elements[1];
    c.quarter = elements[2];
    elements.clear();
...
void write_courses_offered(std::vector<Course>& all_courses) {
  ...
  //write to file
  std::ifstream iss("courses.csv"); std::string header;
  std::getline(iss , header);
  std::ofstream writer("student_output\\courses_offered.csv");
  writer << header << "\n";
  for(size_t i = 0; i < offered_courses.size(); i++)
  {
    writer << offered_courses[i].title << ',' << offered_courses[i].number_of_units << ',' << offered_courses[i].quarter << '\n';
  }
  writer << std::flush;
...
void write_courses_not_offered(std::vector<Course>& unlisted_courses) {
  std::ifstream get_header("courses.csv"); std::string header;
  getline(get_header , header);

  std::ofstream writer("student_output\\courses_not_offered.csv");
  writer << header << '\n';
  for(auto element : unlisted_courses)
  {
    writer << element.title << ',' << element.number_of_units << ',' << element.quarter <<'\n';
  }
  writer << std::flush;
