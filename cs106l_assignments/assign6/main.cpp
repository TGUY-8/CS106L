...
public:
  ...
  std::optional<Course> find_course(std::string course_title)
  {
    ...
    {
      if(cour.title == course_title) return cour;
    }
    return {};
  }
...
main(int argc, char* argv[])
{
  ...
  if (argc == 2) {
    ...
    ********************************************************/
    ...
    std::string output = course ? "Found course: " + course.value().title + ","+ course.value().number_of_units + ","+ course.value().quarter  : "Course not found.";
